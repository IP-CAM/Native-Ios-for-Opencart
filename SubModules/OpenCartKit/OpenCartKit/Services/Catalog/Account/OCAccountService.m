 ...
 */
+ (instancetype)login:(NSString*)email1 password:(NSString*)password1{

    NSString * email = email1;
    NSString* password = password1;

    OCAccountService* service = [[OCAccountService alloc]init];
    NSString* route = @"rest/login/login";
    service.route = route;
    service.method = kHTTP_REQUEST_METHOD_POST;
    
    service.parameters = @{
                           @"email": @"dhruv@guru.com",
                             @"password": @"123456"
    };
    
    
    //OCNSDictionaryOfParametersBindingsJson(email,password);
