...
NSString* DefaultValueOfStoreURL = @"https://www.guruprasadam.com";
...
{
  //  NSString* url = [NSString stringWithFormat:@"%d",AppResourceGet(KeyOfStoreURL, DefaultValueOfStoreURL)];
    return DefaultValueOfStoreURL;
...
+ (void)storeUserAccountInfo:(NSDictionary*) dict{
  //  let encodedData = NSKeyedArchiver.archivedData(withRootObject: category)
    ...
    AppResourceSet(@"App.UserAccount.Info", data);
}

+ (NSDictionary*)loadUserAccountInfo{
    
    NSDictionary* data = [NSKeyedUnarchiver unarchiveObjectWithData:(NSData*)AppResourceGet(@"App.UserAccount.Info", nil)];

    return data;
}

+ (void)storeUserSessionInfo:(NSString*) dict{
    AppResourceSet(@"App.UserAccount.Session", dict);
}

+ (NSString*)loadUserSessionInfo{
    return (NSString*)AppResourceGet(@"App.UserAccount.Session", nil);
}
